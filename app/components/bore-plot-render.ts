...
export interface BuildBorePlotOptions {
...
// Build the bore plot SVG, with hatch patterns injected, ready to append to the
// DOM. Returns null when there are no layers to render.
export function buildBorePlot({
  layers,
  specimens,
  groundwaterLevel,
  width,
  ...
  lang,
  t,
}: BuildBorePlotOptions): (SVGSVGElement | HTMLElement) | null {
  if (layers.length === 0) {
    return null;
  }
  ...
  const plot = Plot.plot({
    ...
    marks: [
      ...
      // Decoded soil descriptions for layers tall enough in pixels. Wrapped to
      // the narrow band column (lineWidth in ems) with a white halo
      // (paint-order: stroke) so longer phrases stay legible over the colours.
      Plot.text(layersWithLabels, {
        x: 0.5,
        y: (d: BoreLayer) => d.depthTop + (d.depthBottom - d.depthTop) / 2,
        text: (d: BoreLayer) => decodeBoreCode(d.soilCode),
        fill: "black",
        stroke: "white",
        strokeWidth: 1.5,
        paintOrder: "stroke",
